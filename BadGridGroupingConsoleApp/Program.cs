...
namespace BadGridGroupingConsoleApp
{
    class Program
    {
        ...
        private static void Mark(int[,] grid, int currentY, int currentX)
        {
            ...
            if (currentX < width && currentY < height)
            {
                ...
                int xIndex = currentX;
                for (int y = currentY + 1; y < width; y++)
                {
                    ++xIndex;
                    if (xIndex < width && grid[y, xIndex] == 1)
                    {
